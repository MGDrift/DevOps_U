# Data Acces Layer
# Abstrae la complejidad de las operaciones de la base de datos y se encarga de manejar las inteacciones de la app y la BD
...
class ToDoList(BaseModel):
    ...
    def from_doc(doc) -> "ToDoList":
        return ToDoList(
            id=str(doc["_id"]),
            name=doc["name"],
            items=[ToDoList.from_doc(item) for item in doc["items"]],
        )
    
class ToDoDAL:
    def __init__(self, todo_collection: AsyncIOMotorCollection):
        self.__todo_collection = todo_collection

    async def list_todo_lists(self, session=None):
        async for doc in self.__todo_collection.find(
            {},
            projection={
                "name": 1,
    ...
    async def create_todo_list(self, name:str, session=None) -> str:
        response = await self.__todo_collection.insert_one(
            {"name": name, "items": []},
            session=session,
        ) 
